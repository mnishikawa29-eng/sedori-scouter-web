...
# ==================== 設定 ====================
# 最終更新: 2026-04-11 (21,491件のデータベース)
...
def get_api_keys():
    ...
    with st.sidebar.expander("🔑 API設定", expanded=False):
        st.markdown("""
        ### Yahoo!ショッピングAPI設定ガイド
        
        **手順**:
        1. [Yahoo!デベロッパーネットワーク](https://developer.yahoo.co.jp/) にアクセス
        2. Yahoo! JAPAN IDでログイン
        3. [アプリケーション登録](https://e.developer.yahoo.co.jp/) で新規アプリ作成
        4. **アプリケーションID（Client ID）** をコピー
        5. 下の入力欄に貼り付け
        ...
        **特徴**:
        ...
        yahoo_app_id_input = st.text_input(
            "Yahoo!アプリケーションID", 
            value=yahoo_app_id,
            type="password",
            help="Yahoo!ショッピング実価格取得用（公式API・無料）",
            placeholder="例: dj00aiZpPWFiY2RlZmdoaWprbG1ub3..."
        )
        
        if yahoo_app_id_input:
            yahoo_app_id = yahoo_app_id_input
        ...
        # APIキーのテスト機能
        if yahoo_app_id and st.button("🧪 API接続テスト", help="サンプルJANでAPI接続を確認"):
            with st.spinner("テスト中..."):
                test_jan = "4902370517392"  # Nintendo Switch Proコントローラー
                price, status, debug_info = search_yahoo_shopping_official(
                    test_jan, yahoo_app_id, debug=True
                )
                
                if price:
                    st.success(f"✅ API接続成功！\n\nテスト商品（JAN: {test_jan}）\n価格: ¥{price:,}\nステータス: {status}")
                else:
                    st.error(f"❌ API接続失敗\n\nステータス: {status}\n\n**トラブルシューティング**:\n- アプリケーションIDが正しいか確認\n- Yahoo!デベロッパーネットワークに登録済みか確認\n- 1分間に30リクエスト以上送信していないか確認")
    ...
    return yahoo_app_id, debug_mode if 'debug_mode' in locals() else False

# ==================== Yahoo!ショッピング価格取得（公式API） ====================
def search_yahoo_shopping_official(jan_code, yahoo_app_id=None, debug=False):
    """Yahoo!ショッピング公式API経由でJANコード検索"""
    debug_info = {}
    
    if not yahoo_app_id:
        return None, "アプリID未設定", debug_info
    
    try:
        url = f"https://shopping.yahooapis.jp/ShoppingWebService/V3/itemSearch?appid={yahoo_app_id}&jan_code={jan_code}&results=30"
        
        # デバッグ情報収集
        if debug:
            debug_info["request_url"] = url.replace(yahoo_app_id, f"{yahoo_app_id[:10]}...")
            debug_info["request_method"] = "GET"
            debug_info["jan_code"] = jan_code
        ...
        if debug:
            ...
            debug_info["response_body"] = res
        
        # エラーチェック
        if "Error" in res:
            error_msg = res["Error"]["Message"]
            if debug:
                debug_info["api_error"] = error_msg
            return None, f"APIエラー: {error_msg}", debug_info
        ...
        if "hits" in res and res["hits"]:
            ...
            if debug:
                debug_info["total_results"] = len(items)
            
            # 最安値を取得
            prices = [int(item["price"]) for item in items if "price" in item]
            
            if prices:
                min_price = min(prices)
                if debug:
                    debug_info["prices_found"] = len(prices)
                    debug_info["min_price"] = min_price
                return min_price, "成功", debug_info
            else:
                return None, "価格情報なし", debug_info
        else:
            return None, "商品なし", debug_info
    
    except Exception as e:
        if debug:
            debug_info["exception"] = str(e)
        return None, f"エラー: {str(e)}", debug_info
...
# ==================== ランキング生成 ====================
def create_ranking_df(buyback_db, config, selected_category="すべて", limit=50, yahoo_app_id=None, debug=False):
    """利益ランキングを生成（Yahoo!公式API対応版）"""
    
    if not yahoo_app_id:
        st.error("❌ Yahoo!アプリケーションID が設定されていません。左サイドバーの「🔑 API設定」から設定してください。")
        return pd.DataFrame()
    ...
    for idx, (jan, info) in enumerate(filtered_items):
        ...
        # Yahoo!価格取得
        yahoo_price, api_status, _ = search_yahoo_shopping_official(jan, yahoo_app_id, debug=False)
...
def main():
    st.set_page_config(
        page_title="せどり利益スカウター v7.0",
        page_icon="🔍",
        layout="wide"
    )
    
    st.title("🔍 せどり利益スカウター v7.0")
    st.caption("Yahoo!ショッピング公式API対応版 - JANコード完全一致検索")
    ...
    # APIキー取得
    yahoo_app_id, debug_mode = get_api_keys()
    
    # APIキー警告
    if not yahoo_app_id:
        st.error("""
        ### 🚨 Yahoo!アプリケーションID が設定されていません
        
        Yahoo!ショッピングの実価格を取得するには、Yahoo!デベロッパーネットワークへの登録が必要です。
        """)
        ...
        st.info("""
        **登録手順（5分）**:
        1. 上の「Yahoo!デベロッパーネットワークに登録」ボタンをクリック
        2. Yahoo! JAPAN IDでログイン
        3. 「アプリケーション登録ページ」で新規アプリ作成
        4. **アプリケーションID（Client ID）** をコピー
        5. 左サイドバー「🔑 API設定」に貼り付け
        6. 「🧪 API接続テスト」で動作確認
    ...
    with st.sidebar.expander("📊 表示設定", expanded=False):
        ...
        calc_limit = st.number_input("計算対象件数", 10, 500, 30, 10, 
                                     help="処理時間目安: 30件=約1分, 100件=約3分30秒（API制限: 30リクエスト/分）")
    ...
    # ランキング生成ボタン
    if st.button("🚀 ランキングを生成", type="primary", disabled=not yahoo_app_id):
    ...
    st.markdown("---")
    st.info(f"""
    ### 🔥 v7.0 Yahoo!ショッピング公式API対応版
    
    **✅ 新機能**:
    - **🛒 Yahoo!ショッピング公式API**: JANコード完全一致検索に対応
    - **💰 完全無料**: アプリケーションID登録のみで利用可能
    - **🎯 高精度価格取得**: RapidAPIの404エラーを完全解決
    - **📊 1分間に30リクエスト**: 無料で安定利用
    ...
    **処理時間目安**:
    - 30件: 約1分（API制限: 30リクエスト/分）
    - 100件: 約3分30秒
    - 300件: 約10分30秒
    
    **Yahoo!デベロッパー登録（5分）**:
    1. [Yahoo!デベロッパーネットワーク](https://developer.yahoo.co.jp/) を開く
    2. Yahoo! JAPAN IDでログイン
    3. [アプリケーション登録](https://e.developer.yahoo.co.jp/) で新規アプリ作成
    4. アプリケーションID（Client ID）をコピー
    5. 左サイドバー「🔑 API設定」に貼り付け
    ...
    **公式ドキュメント**:
    - [Yahoo!ショッピングAPI仕様](https://developer.yahoo.co.jp/webapi/shopping/v3/itemsearch.html)
    - [利用規約](https://developer.yahoo.co.jp/webapi/shopping/api_contract.html)
    """)
