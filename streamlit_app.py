...
# ==================== ランキング生成（ダミーモード対応） ====================
def create_ranking_df(buyback_db, config, selected_category="すべて", limit=50, rapidapi_key=None, rakuten_app_id=None, dummy_mode=False):
    """利益ランキングを生成（カテゴリフィルタ対応）"""
    results = []
    api_success_count = 0
    ...
    for idx, (jan, info) in enumerate(filtered_items):
        status_text.text(f"処理中... {idx+1}/{len(filtered_items)} ({selected_category})")
        progress_bar.progress((idx + 1) / len(filtered_items))
        
        # ダミーモード：実APIを呼ばず推定値のみで計算
        if dummy_mode:
            yahoo_price = None  # 推定価格を使用
        else:
            # Yahoo!価格取得
    ...
    if not df.empty:
        if dummy_mode:
            st.info(f"ℹ️ ダミーモード: 推定価格で計算（実APIは使用していません）")
        else:
            st.success(f"✅ Yahoo!実価格取得: {api_success_count}/{len(filtered_items)} 件")
...
def main():
    st.set_page_config(
        page_title="せどり利益スカウター v6.0",
        page_icon="🔍",
        layout="wide"
    )
    
    st.title("🔍 せどり利益スカウター v6.0")
    st.caption("カテゴリ選択 & ダミーモード対応版 (GitHub Releases連携)")
    ...
    dummy_mode = False
    if not rapidapi_key:
        st.warning("⚠️ RapidAPI Key が設定されていません。ダミーモードで動作します（推定価格のみ）")
        dummy_mode = True
    ...
    # ランキング生成ボタン
    button_label = "🚀 ランキングを生成 (ダミーモード)" if dummy_mode else "🚀 ランキングを生成"
    if st.button(button_label, type="primary"):
        with st.spinner(f"利益商品を検索中...（カテゴリ: {selected_category}）"):
        ...
        if df.empty:
            ...
            # テーブル表示
            st.dataframe(
                df[['category', 'jan_code', 'buyback_price', 'display_price', 'earned_points', 
            ...
            st.download_button(
                "📥 CSV ダウンロード",
                csv,
                f"profit_ranking_{selected_category}_{datetime.now().strftime('%Y%m%d')}.csv",
                "text/csv"
            )
    ...
    st.markdown("---")
    st.info(f"""
    ### 🔒 v6.0 カテゴリ選択 & ダミーモード対応版
    
    **✅ 新機能**:
    - カテゴリ別検索対応（カメラ・ゲーム・家電等）
    - JANプレフィックス分布表示
    - ダミーモード（APIキーなしでテスト可能）
